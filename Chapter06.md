...
用户必须告知软件包授权方式。复制软件包，给你的朋友是否合法？你是否可以修改代码和发布代码？所有信息都会在COPYING文件中提供。

正如上面提到的，我们应该使用GNU General Public第3版或者更新的版本。网页，你可以读到更多该授权有关信息。

通常你会将授权会写进每个m文件。

**INDEX文件**

软件包包含3个基本函数，即，和一个封装好的函数。我们可以在软件包中用INDEX文件来指明文件：

        # Index file for pois-solv

        toolbox >>pois-solv
        Basic functions
          cmat_1d.m cmat_2d vecmat_convert.m
        Wrapper functions
        pois_fd.m
...
我假设我们已经保存好所有软件包相关文件（m文件和）至同一个目录。我们通过下面步骤来构建软件包：

1. 生成软件包目录：

        octave:26> mkdir "poissolv";

2. 生成inst目录：

        octave:28> mkdir "poissolv/inst";

3. 复制文件值软件包目录，比如：

        octave:29> copyfile("DESCRIPTION","posissolv");

4. 复制m文件值inst目录：

        octave:30> copyfile("cmat_1d.m","poissolv/inst/")

5. 存档和压缩软件包目录：

        octave:31> tar("pois-solv.tar","poissolv");
        octave:32> gzip("pois-solv.tar");

6. 最后，构建Octave软件包：

        octave:33> pkg build ./ pois-solv.tar.gz

我们使用第1章中的软件包管理器命令pkg。当我们讨论如何安装软件包，它也可以用于构建软件包。命令31会生成文件，这就是你的Octave软件包。

你现在可以尝试按转自己的软件波。我们已经在第1章里解释了如何来做。如果你需要做更多复杂的，还有许多的特征可以使用。比如，软件包依赖于其他软件包的某些版本，可能还有C++或者fortran代码，这些代码需要编译。咨询Octave手册，如果你需要更为高级的软件包构建过程。

**软件包的限制和可能的扩展**

当然，还有空间来对软件包进行改进和扩展。比如:

1. cmat_1d和cmat_2d没有优化，因为他们中稀疏矩阵的初始化需要对内存重置。因此，函数对于大规模的网格数是非常慢。上面的练习中，我们显示如果进行修复。

2. 我们使用Octave的内置线性方程求解方法。尽管快速，我们可以使用不同的方法来降低运行时间。比如，对于大规模计算，可以使用relaxation方法。

3. 我们可以使用更高次的有限元来提高准确度。

4. 通过实现坐标转换方法，软件包可以支持不同的几何学。

5. 我们完全忽视3维Poisson方程。对软件包的一个自然的扩展是包含这部分方法。

6. 我们可以实现其他类型的边界条件。

7. 使用更为高级和精确的数值技术比如有限元法来实现。

你自己垦能会有更多的东西会加入清单。我们如果需要对软件包进行改进，我们应当首先思考一下如何开始。
